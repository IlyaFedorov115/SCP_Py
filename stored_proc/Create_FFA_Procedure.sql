...
CREATE PROCEDURE FFA_Algorithm  (
		...
		@numIter INT,
		@vectorDist VARCHAR(20) = 'euclid',	-- тип расстояния
		@notationType VARCHAR(5) = 'CS', 	-- нотация CS - столбцы покрывают строки, SС - строки покрывают столбцы
		@transferFun VARCHAR(5) = 'S1',
		@rowName VARCHAR(5) = 'Row',
		@colName VARCHAR(5) = 'Col',
		@move_type VARCHAR(20) = 'standart', -- разновидность перемещения светлячков
...
DECLARE @cost_ind VARCHAR(5)
DECLARE @requestTable NVARCHAR(max) -- Задаем запрос на таблицу покрытия со значениями стоимостей

IF @notationType = 'SC' OR @notationType = 'sc'
	BEGIN
	PRINT 'SC notation'
	SET @cost_ind = @rowName
	END
ELSE
	BEGIN
	PRINT 'CS notation'
	SET @cost_ind = @colName
	END


SET @requestTable = 'SELECT ['+@tableName+'].['+@rowName+'], ['+@tableName+'].['+@colName+'], ['+@costsName+'].Cost 
	FROM ['+@tableName+'] LEFT JOIN ['+@costsName+'] ON ['+@tableName+'].['+@cost_ind+'] = ['+@costsName+'].Ind';
SET @resultCost = 0;


BEGIN
	-- создать таблицу результат (просто набор номеров вошедших рядов)
	IF NOT EXISTS (SELECT * FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_NAME = 'FFA_Algo_Result')
	BEGIN
		PRINT 'NOT EXISTS'
		Create Table FFA_Algo_Result (Row_ INT);
	END
	TRUNCATE TABLE FFA_Algo_Result;


	-- применение самого алгоритма. Само решение запишется в таблицу FFA_Algo_Result
	-- а результирующая стоимость выведена на экран и возвращена через OUTPUT переменную @resultCost
	INSERT INTO FFA_Algo_Result 
...
if notation.lower() == "sc":
	cover_rows = np.unique(InputDataSet[rowName])
else:
	cover_rows = np.unique(InputDataSet[colName])

tmp_lst = InputDataSet[colName].tolist()
costs = np.array(InputDataSet["Cost"][[tmp_lst.index(e) for e in cover_rows]])
table = np.array(InputDataSet[[rowName, colName]])#.to_numpy()
...
optimum, values = ffa_algorithm(table, costs, pop_size=pop_size, max_iter=max_iter, gamma=gamma, betta_0=betta_0, notation=notation,
                                                    transfer_fun=transfer_fun, distance=vectorDist, betta_pow=2,
                                                    alpha=alpha, alpha_inf=alpha_inf, alpha_0=alpha_0, gamma_alter=gamma_alter, move_type=move_type)
...
OutputDataSet = optimum
	...
	, @params = N'@resultCost INT OUTPUT, @popSize INT, @numIter INT, @vectorDist VARCHAR(20), @notationType VARCHAR(5), @transferFun VARCHAR(5), @rowName VARCHAR(5), @colName VARCHAR(5), @move_type VARCHAR(20), @gamma FLOAT, @gamma_alter INT, @betta_0 FLOAT, @alpha FLOAT, @alpha_inf FLOAT, @alpha_0 FLOAT'
...
END;
	...
	-- @move_type VARCHAR(20), @gamma FLOAT, @gamma_alter INT, @betta_0 FLOAT, @alpha FLOAT, @alpha_inf FLOAT, @alpha_inf FLOAT
